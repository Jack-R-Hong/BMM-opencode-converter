...
import type { OpenCodeAgent, OpenCodeSkill } from '../types/index.js';

// .agents/ format — cross-IDE compatible
// Uses OpenCode types directly (same frontmatter structure)

export function writeAgentsAgent(outputDir: string, agent: OpenCodeAgent): void {
  const agentDir = path.join(outputDir, '.agents', 'agents');
...
export function writeAgentsSkill(outputDir: string, skill: OpenCodeSkill): void {
  const skillDir = path.join(outputDir, '.agents', 'skills', skill.folder);
